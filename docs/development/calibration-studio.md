...
Since 2.7.10.3 the Studio button opens a login dialog before the floating modal is created. The flow is:

```text
Studio button -> login dialog -> calibrationAuthenticate -> Studio modal -> calibrationBootstrap
```

`calibrationBootstrap` is never sent before a successful employee verification. The PIN stays only in the Angular in-memory admin session and is not written to browser storage, logs, documents, database rows or generated files. The Views tab no longer has an independent username/PIN form; it shows the current session and only keeps the change reason used for write actions.
...
Since 2.7.10.3 `calibrationBootstrap` also requires the verified in-memory admin session. Writes require the same employee verification payload plus a change reason. The API uses revision checks; saving stale rows returns a conflict instead of overwriting newer calibration work.
...
Authentication failures (`401`/`403` or the safe auth error codes returned by the endpoint) clear the in-memory credentials and reopen the login dialog. The original Studio request is retried at most once after a successful reauthentication, so an expired PIN cannot create an endless request loop.
