...
require("./pdf/Montserrat-Regular-normal.js");
require("./pdf/Montserrat-SemiBold-normal.js");
...
export class AppComponent implements OnInit {
  ...
  state = {
    ...
    dbSaveItems: [] as iDBSaveItem[], // Liste aller zur ID gehörigen Presets
    toolsMenu: false, // show tools overlay
    ringDetails: false, // show ring details overlay
    logVisible: false, // Logger visible (Anpassungsmeldungen)
    ...
    diamond: false,
    freeStones: false,
    preset_id: "", // if not URL-set then dbGetId() result
    browsertab_id: "",
  };
  ...
  //       divPreset: "-:1"
  //       // Fallback für rwMin: Hier darf keine Mindestbreite angegeben werden
  //     },
  //     {
  //       name: "Zweifarbig",
  ...
  //     }
  //   ],
  //   material: [ // Aktuelle Farben: abgeglichen am 23.02.2023
  //     {
  //       id: 0,
  //       name: "Gelbgold",
  //       symbol: "Au",
  ...
  //         uScale: 0.142857142, // 1.0/7 => nur Ganzzahlige Teiler
  //         vScale: 0.142857142,
  //         // vScale: 0.14,
  //         invertX: true,
  //         invertY: true,
  ...
  //
  //   /**
  //    *   rowWidth = stoneSize * numRows + stoneDistances.stoneToStone_x * (numRows - 1) + stoneToBevel_x * 2;
  //
  //    */
  //   stoneMode: [
  //     {
  ...
  //       stoneDistances: [
  //         {
  //           profile: ["P1", "P2", "P3", "P4", "P5", "P6", "P11", "P7", "P12"],
  //           stoneToStone_x: 0,
  //           stoneToStone_y: 50,
  //           stoneToGap_x: 100,
  //           stoneToBevel_x: -0.12 // 12% kleiner als der Stein TODO: wird noch nicht ausgewertet
  //         },
  //       ]
  //     },
  //     {
  ...
  //     // {
  //     //   mode: 36,
  //     //   name: "Spannring schräg",
  //     //   img: "icon-stone-clamp-diagonal.svg",
  //     //   safeDistX: 200,
  //     //   safeDistY: 200,
  //     //   safeProfileSideDist: 150,
  //     //   minRingWidth: 3000,
  //     //   alternativeMode: 35
  //     // },
  //     {
  ...
  //       obj: 'brillant.obj',
  //       allowedStoneMode: [10, 11, 20, 30, 31, 35, 36, 40, 41, 42, 43, 44, 45],
  //       size: [
  //         {
  //           size: 1000,
  //           carat: 0.005,
  ...
  //         }
  //       ],
  //       // distributionFactor: 1.414213562,
  //       sizeDepthFactor: 0.73,
  //     },
  //     {
  //       id: 4,
  ...
  //     tesselation: [150, 100, 20],
  //     ringRotationX: 27,
  //     ringRotationY: [42, 0],
  //     ringOffsetZ: [-2.5, 0],
  //     camera: [-1.8830285392434571, 1.1869035161232264, 30],// für Ortho-Camera
  //     // camera: [-2.079124231498639, 1.2150118627962285, 30],// für Ortho-Camera
  //     // camera: [-2.079124231498639, 1.2150118627962285, 63],// Werte aus V1 (ausgelesen via console)
  //     cameraMinOrthoSize: 20,
  //     // cameraMinOrthoSize: 17,
  //     // sceneExposure: 2.0,
  //     // sceneContrast: 1.75,
  //     // envRotationRad_yaw: 1.1938052083641213,
  //     // envRotationRad_pitch: 6.283185307179586,
  //     forceFrames: 15,
  //     maxFps: 40,
  //
  //     environmentPreset: {
  //       refSampler_image: "diamondMap_0.jpg",
  //       refSampler_reflect: 1.811,
  //       refSampler_camRad: 1.394,
  //       refSampler_factor: 1.0,
  //
  //       tri1_reflect: 1.0,
  //       tri1_camRad: 0.5,
  //       tri1_factor: 0.0,
  ...
  //       fire_factor: 0.0,
  //
  //       envTexture_yaw: 0.18999999999999997921184907018485,
  //       envTexture_pitch: 0.99999999999999992409498312558244,
  //       envTexture_roll: 0.0,
  //
  //       scene_exposure: 2.0,
  //       scene_contrast: 1.75,
  ...
  // neue AppData von vonjacob.de vom 24.11.2024
  // Grund war die implementierung von Silber
  data: iAppData =
    {
      "profile": [
        {
  ...
  ngOnInit() {
    dbGetAppData().then(function (data: any) {
      // if (AppComponent.app.state.debug) console.log("DEBUG_MODE: NUTZE INTERNE KONFIGURATIONSDATEN!");
      // else
      {
        if (data == null) Log("error", "Keine App Daten vorhanden!"); // ausgelagert in admin-Bereich
  ...
  createPdf() {
    ...
    dbSavePreset().then(function () {
      ...
      collectRingScreenshots(800, 622, function (data: string[]) {
        ...
        // pdf.addImage(a4pdfbg, 'PNG', 0, 0, 210, 297);
...
export async function dbLoadPreset(id: string): Promise<iDBSaveItem | undefined> {
  ...
  // urlId überschreibt Standardpreset
  // @ts-ignore
  if (id == "0000-0000" && AppComponent.app.state.urlParams["id"] != undefined) { // @ts-ignore
  ...
  if (id == "0000-0000") {
    // existiert ein Eintrag in localStorage
    // let localId: string = localStorage.getItem("ringconfId") || "";
    // if (localId.length >= 9)
    // {
    //   let done = false;
    //   await dbCheckIdExist(localId).then(function (exist: boolean)
    //   {
    //     if (exist)
    //     {
    //       AppComponent.app.state.dbSaveItems = [];
    //       dbLoadPreset(localId);
    //       done = true;
    //       Log("info", "Es wurde die zuletzt gespeicherte Konfiguration geladen.", "Info");
    //     }
    //     else
    //     {
  ...
  /*
    else // ...nicht in der dbSaveItems Liste enthalten oder ungültig
    {
      let url = getDistRootUrl();
      let headers = makeHttpHeaders();
      let params = makeHttpParams("dbLoadPreset", [id]);
      ...
      await lastValueFrom(response).then(function (data: any)
      {
        if (data.errorCode !== 0)
        {
          switch (data.errorCode)
          {
            ...
            {
              Log("error", data.error);
              Log("error", "ACHTUNG: Fehlerhafte Datenbank. Standardkonfiguration wurde nicht in der Datenbank gefunden.");
        ...
        else
        {
          ...
          if (id !== "0000-0000")
          {
            ...
            if (data.dbItems)
            {
              ...
              items.forEach(function (e)
              {
                dbLoadPreset(e.id, true).then(function (data: any)
                {
                  if (data)
                  {
                    AppComponent.app.state.dbSaveItems.push({
      ...
      //         Log("error", data.error);
      //         Log("error", "ACHTUNG: Fehlerhafte Datenbank. Standardkonfiguration wurde nicht in der Datenbank gefunden.");
      //         Log("info", "Es wird versucht, das Standardtemplate wieder herzustellen");
      //
      ...
      //           RingData.list[0].isDirty = true;
      //           RingData.list[1].isDirty = true;
      //         }, "dbSavePreset", ["0000-0000", RingData.list[0], RingData.list[1], "", true]);
      //         break;
      //       }
      //       case -2:
      //       {
      ...
      //     if (id !== "0000-0000")
      //     {
      //       // erstelle die state.dbSaveItems Liste
      //       item = {
      //         id: id,
      //         preset_0: data.preset_0,
      //         preset_1: data.preset_1,
      ...
      //     else
      //     {
      //       dbGetId().then(function (data: any)
      //       {
      //         if (AppComponent.app.state.debug)
      //           console.log("dbGetId: ", data.id);
      //         AppComponent.app.state.preset_id = data.id;
      //         AppComponent.app.state.dbSaveItems = [];
      //         AppComponent.app.state.ready = true;
      //       })
...
export async function dbSaveStdPreset() {
  ...
  await lastValueFrom(response).then(function (data: any) {
    if (data.errorCode === 0)
      ...
      Log("eror", "Standardpreset konnte nicht gesetzt werden. Errorcode " + data.errorCode);
  });
...
export async function restoreEnvTexture() {
  ...
  await lastValueFrom(response).then(function (data: any) {
    if (data.errorCode == 0) {
      ...
      console.log(data);
      Log("error", "Daie Umgebungsmap konnte nicht wieder hergestellt werden!");
    }
  })
...
export async function dbSaveEnvironmentPreset(name: string, json_iEnvironmentPreset: string) {
  ...
  await lastValueFrom(response).then(function (data: any) {
    if (data.errorCode === 0)
      ...
      Log("eror", "Environment-Preset konnte nicht gespeichert werden. Errorcode " + data.errorCode);
  });
}
